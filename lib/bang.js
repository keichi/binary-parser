...
Parser.prototype.generateNextParser = function(code) {
    if (this.next) {
        code += this.next.generate();
    }
...
Object.keys(PRIMITIVE_TYPES).forEach(function(type) {
    Parser.prototype['generate' + type] = function() {
        var code = '';

        code += util.format('vars.%s = buffer.read%s(iterator);\n', this.varName, type);
        code += util.format('iterator += %d;\n', PRIMITIVE_TYPES[type]);

        return this.generateNextParser(code);
    };
});

Parser.prototype.generateField = function(val) {
    if (typeof val === 'number') {
        return val.toString();
    } else if (typeof val === 'string') {
        return util.format('vars.%s', val);
    } else if (typeof val === 'function') {
        return util.format('(%s)(vars)', val);
    }
};

Parser.prototype.generateSkip = function() {
    var code = util.format('iterator += %d;\n', this.options.length);

    return this.generateNextParser(code);
};

Parser.prototype.generateString = function() {
    var code = '';
    code += util.format('vars.%s = buffer.toString(\'%s\', iterator, iterator + %s);\n',
                                this.varName,
                                this.options.encoding,
                                this.generateField(this.options.length)
                            );
    code += util.format('iterator += %s;\n', this.generateField(this.options.length));
    
    return this.generateNextParser(code);
};

Parser.prototype.generateArray = function() {
    var length = this.generateField(this.options.length);
    var type = this.options.type;
    var code = '';

    if (typeof type === 'string') {
        code += util.format('vars.%s = [];\n', this.varName);
        code += util.format('for (var i = 0; i < %s; i++) {\n', length);
        code += util.format('\tvars.%s.push(buffer.read%s(iterator));\n', this.varName, NAME_MAP[type]);
