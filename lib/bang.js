...
Object.keys(PRIMITIVE_TYPES).forEach(function(type) {
    Parser.prototype['generate' + type] = function(ctx) {
        ctx.code += util.format('%s = buffer.read%s(iterator);\n', this.generateVariable(ctx, this.varName), type);
...
Parser.prototype.generateVariable = function(ctx, name) {
    var arr = ['vars'];
    Array.prototype.push.apply(arr, ctx.scopes);
    if (name) {
        arr.push(name);
    }
...
Parser.prototype.generateOption = function(ctx, val) {
    if (typeof val === 'number') {
        return val.toString();
    } else if (typeof val === 'string') {
        return this.generateVariable(ctx, val);
    } else if (typeof val === 'function') {
        return util.format('(%s).call(%s)', val, this.generateVariable(ctx));
    }
};

Parser.prototype.generateSkip = function(ctx) {
    var length = this.generateOption(ctx, this.options.length);
...
Parser.prototype.generateString = function(ctx) {
    ctx.code += util.format('%s = buffer.toString(\'%s\', iterator, iterator + %s);\n',
                                this.generateVariable(ctx, this.varName),
                                this.options.encoding,
                                this.generateOption(ctx, this.options.length)
                            );
    ctx.code += util.format('iterator += %s;\n', this.generateOption(ctx, this.options.length));
...
Parser.prototype.generateArray = function(ctx) {
    ...
    var lhs = this.generateVariable(ctx, this.varName);

    if (typeof type === 'string') {
        ctx.code += util.format('%s = [];\n', lhs);
        ctx.code += util.format('for (var %s = 0; %s < %s; %s++) {\n', counter, counter, length, counter);
        ctx.code += util.format('%s.push(buffer.read%s(iterator));\n', lhs, NAME_MAP[type]);
        ctx.code += util.format('iterator += %d;\n', PRIMITIVE_TYPES[NAME_MAP[type]]);
        ctx.code += '}\n';
    } else if (type instanceof Parser) {
        ctx.code += util.format('%s = [];\n', lhs);
        ctx.code += util.format('for (var %s = 0; %s < %s; %s++) {\n', counter, counter, length, counter);
        ctx.code += util.format('%s.push({});\n', lhs);
...
Parser.prototype.generateChoice = function(ctx) {
    var tag = this.generateOption(ctx, this.options.tag);
    var type = this.options.choices[tag];
    var lhs = this.generateVariable(ctx, this.varName);
    var tmp;

    ctx.code += util.format('%s = {};\n', lhs);
    ctx.code += util.format('switch(%s) {\n', tag);
    Object.keys(this.options.choices).forEach(function(tag) {
        var type = this.options.choices[tag];

        ctx.code += util.format('case %d:\n', tag);
        if (typeof type === 'string') {
            ctx.code += util.format('%s = buffer.read%s(iterator);\n', lhs, NAME_MAP[type]);
            ctx.code += util.format('iterator += %d;\n', PRIMITIVE_TYPES[NAME_MAP[type]]);
        } else if (type instanceof Parser) {
