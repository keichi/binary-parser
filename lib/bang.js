...
Parser.prototype._parse = function(context) {
    return this['_parse' + this.type](context);
};

Parser.prototype.parseNext = function(context) {
    return this.next ? this.next._parse(context) : context;
};

Object.keys(PRIMITIVE_TYPES).forEach(function(type) {
    Parser.prototype['_parse' + type] = function(context) {
        context.vars[this.varName] = context.buffer['read' + type](context.iterator);
        context.iterator += PRIMITIVE_TYPES[type];

        return this.parseNext(context);
    };
});

Parser.prototype._parseSkip = function(context) {
    context.iterator += this.options.length;

    return this.parseNext(context);
};


Parser.prototype.evalField = function(val, context) {
    if (typeof val === 'number') {
        return val;
    } else if (typeof val === 'string') {
        if (val in context.vars) {
            return context.vars[val];
        } else {
            throw new Error('Field ' + val + ' is not yet defined.');
        }
    } else if (typeof val === 'function') {
        return val.call(context.vars);
    }
};

Parser.prototype._parseString = function(context) {
    var encoding = this.options.encoding;
    var length = this.evalField(this.options.length, context);

    context.vars[this.varName] = context.buffer.toString(encoding, context.iterator, context.iterator + length);
    context.iterator += length;

    return this.parseNext(context);
};

Parser.prototype._parseArray = function(context) {
    var length = this.evalField(this.options.length, context);
    var type = this.options.type;
    var i, data, ctx;

    context.vars[this.varName] = [];
    if (typeof type === 'string') {
        for (i = 0; i < length ; i++) {
            ctx = this['_parse' + NAME_MAP[type]].call(
                new Parser(), {
...
Parser.prototype._parseChoice = function(context) {
    var tag = this.evalField(this.options.tag, context);
    ...
    if (typeof type === 'string') {
        ...
        context.iterator = ctx.iterator;
        context.vars[this.varName] = ctx.vars[''];
    } else if (type instanceof Parser) {
        ctx = type.next._parse({
            buffer: context.buffer,
            iterator: context.iterator,
