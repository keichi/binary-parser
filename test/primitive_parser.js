  ...
  describe(`Primitive parser (${Buffer.name})`, function () {
    ...
    describe("Primitive parsers", function () {
      it("should nothing", function () {
        ...
        const buffer = Buffer.from([0xa, 0x14, 0x1e, 0x28, 0x32]);
    ...
    describe("Bit field parsers", function () {
      ...
      it("should parse 1-byte-length bit field sequence", function () {
        ...
        assert.deepStrictEqual(parser1.parse(buf), {
          a: 1,
          b: 2,
          c: 10,
          d: 0,
        ...
        assert.deepStrictEqual(parser2.parse(buf), {
          a: 0,
          b: 2,
          c: 10,
          d: 1,
    ...
    describe("Buffer parser", function () {
      it("should parse as buffer", function () {
        ...
        const result = parser.parse(Buffer.from([...Buffer.from([8]), ...buf]));
      ...
      it("should parse until function returns true when readUntil is function", function () {
        ...
        assert.deepStrictEqual(result1, { cmd: 0xaa, data: Buffer.from([]) });

        const result2 = parser.parse(hexToBuf("aabbcc"));
        assert.deepStrictEqual(result2, { cmd: 0xaa, data: hexToBuf("bbcc") });

        const result3 = parser.parse(hexToBuf("aa02bbcc"));
        assert.deepStrictEqual(result3, { cmd: 0xaa, data: Buffer.from([]) });

        const result4 = parser.parse(hexToBuf("aabbcc02"));
        assert.deepStrictEqual(result4, { cmd: 0xaa, data: hexToBuf("bbcc") });

        const result5 = parser.parse(hexToBuf("aabbcc02dd"));
        assert.deepStrictEqual(result5, { cmd: 0xaa, data: hexToBuf("bbcc") });
      });
